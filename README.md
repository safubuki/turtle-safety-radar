...
- Parent Console
  オンボーディング、保護者用 PIN、ホーム、ログ一覧、外部見守りチェック、設定
- Notification Monitor
  監視対象アプリの通知を解析し、リスク判定を実行
- Text Watcher (Accessibility)
  どのキーボードを使っていても入力テキストをリスク判定 (高リスク時のみ最小限ログ)
- Safety IME
  送信前チェック用の軽量 IME パネル
- Media Checker
  画像選択による OCR と QR 解析。スクリーンショット自動検査の前景サービス
- Local AI
  ルールベース判定に加える軽量な文脈補正モード
- Core
  リスク判定、設定管理、PIN、Room ログ DB、保持期間制御、権限取り消し検知
...
初回起動時はウェルカム画面で機能とプライバシー方針を案内したのち、保護者用 PIN のセットアップに進みます。
PIN 解除後は次の 4 画面を使えます。

- ホーム
  全体ステータス、初期セットアップ進捗、入力監視 / 通知監視 / 保護者通知 / 画像チェッカー / Safety IME の状態、検知ログ件数、開発者向け診断 (セルフテスト、文章判定) を確認
- ログ
  検知ログの一覧表示 (リスクレベル別フィルタ、確認済み化、共有、全削除)
- チェック
  External Guard の確認項目 (21 項目を 4 カテゴリで管理)
- 設定
  検知感度、監視対象アプリの追加 / 削除、画像チェッカー、Local AI、データ保持、プライバシー方針
...
2. アプリを起動し、ウェルカム画面で機能とプライバシー方針を確認する
3. 保護者用 PIN を設定する
4. ホーム画面で初期セットアップ進捗を確認しながら、次を順に許可する
   - 入力監視 (ユーザー補助サービス)
   - 通知アクセス
   - Android 13 以降では保護者通知 (POST_NOTIFICATIONS)
5. 必要なら設定画面で監視対象アプリの追加 / 削除、検知感度、画像チェッカー、Local AI を調整する
6. 「チェック」画面で外部の見守り (Family Link 等) の併用状態を点検する

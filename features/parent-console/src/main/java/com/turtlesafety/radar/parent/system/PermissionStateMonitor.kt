...
class PermissionStateMonitor(
    ...
    suspend fun snapshotAndReport() {
        ...
        if (previous.safetyImeIsDefault && current.safetyImeEnabled && !current.safetyImeIsDefault) {
            recordRevocation(label = "Safety IME 以外が標準入力方式になりました")
        }
    ...
    private fun storedSnapshot(): Snapshot = Snapshot(
        ...
        safetyImeIsDefault = prefs.getBoolean(KEY_IME_DEFAULT, false),
    )
    ...
    private fun storeSnapshot(snapshot: Snapshot) {
        prefs.edit()
            ...
            .putBoolean(KEY_IME_DEFAULT, snapshot.safetyImeIsDefault)
            .apply()
    }
    ...
    private data class Snapshot(
        ...
        val safetyImeIsDefault: Boolean,
    )
    ...
    companion object {
        ...
        private const val KEY_IME_DEFAULT = "safety_ime_default"

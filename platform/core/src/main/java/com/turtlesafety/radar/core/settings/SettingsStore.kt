...
class SettingsStore(context: Context) {
    ...
    var mediaCheckerEnabled: Boolean
        get() = prefs.getBoolean(KEY_MEDIA_CHECKER_ENABLED, true)
        set(value) {
            prefs.edit().putBoolean(KEY_MEDIA_CHECKER_ENABLED, value).apply()
        }

    var localAiMode: LocalAiMode
        get() = LocalAiMode.fromName(prefs.getString(KEY_LOCAL_AI_MODE, null))
        set(value) {
            prefs.edit().putString(KEY_LOCAL_AI_MODE, value.name).apply()
        }
    ...
    companion object {
        ...
        private const val KEY_MEDIA_CHECKER_ENABLED = "media_checker_enabled"
